...
function getCategoryLabel(id: CategoryId, t: ReturnType<typeof useT>): string {
  return t.notesScreen.categories[id] ?? id;
}
        ...
        >
          ...
          <View style={{ paddingHorizontal: 20, paddingTop: 8, paddingBottom: 16 }}>
            ...
            <Text style={{ fontSize: 13, color: colors.ink.faint, marginTop: 4 }}>
              {notes.length > 0
                ? t.notesScreen.tasksSuffix(notes.filter((n) => !n.done).length, notes.filter((n) => n.done).length)
                : t.notesScreen.startWriting}
            </Text>
          </View>
          ...
          >
            {NOTE_CATEGORIES.map((cat) => {
              ...
              const label = getCategoryLabel(cat.id, t);
              return (
                <Pressable
                  key={cat.id}
          ...
          >
            {visibleNotes.length === 0 ? (
              <EmptyState
                ...
                noNotesText={t.notesScreen.noNotes}
                tapToAddText={t.notesScreen.tapToAdd}
                addNoteText={t.notesScreen.addNote}
                  ...
                  <>
                    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10, marginTop: 24, marginBottom: 12 }}>
                      ...
                      <Text style={{ fontSize: 11, color: colors.ink.faint, textTransform: 'uppercase', letterSpacing: 0.8 }}>
                        {t.notesScreen.doneSection(doneNotes.length)}
                      </Text>
                      <View style={{ flex: 1, height: 1, backgroundColor: colors.line.DEFAULT }} />
                    </View>
          ...
          <View style={{
            ...
            <View style={{
              ...
              <TextInput
                ...
                placeholder={t.notesScreen.addToCategory(getCategoryLabel(activeCategory, t))}
