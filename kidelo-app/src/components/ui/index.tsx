...
export type PillTone = 'sage' | 'clay' | 'evergreen' | 'mustard' | 'neutral';
...
export function Checkbox({ checked, onPress, size = 22 }: CheckboxProps) {
  return (
    <Pressable
      onPress={onPress}
      className={`items-center justify-center rounded-md border-1.5 ${
        checked ? 'bg-evergreen border-evergreen' : 'bg-cream border-line-strong'
      }`}
      style={{ width: size, height: size }}
    >
      {checked && <Icon name="check" size={size - 8} color={colors.cream.DEFAULT} strokeWidth={2.4} />}
    </Pressable>
  );
}
