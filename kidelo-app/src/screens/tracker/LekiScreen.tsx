...
const REWARDS = [
  { id: 'streak_7',  icon: '🌱', label: '7 dni regularności',      minStreak: 7  },
  { id: 'streak_30', icon: '🔥', label: '30 dni regularności',     minStreak: 30 },
  { id: 'streak_90', icon: '🌟', label: '90 dni bez pominięcia',   minStreak: 90 },
  { id: 'month',     icon: '💎', label: 'Idealny miesiąc',         minStreak: 0, minMonth: 100 },
];
  ...
  return (
    <SafeAreaView edges={['top']} style={{ flex: 1, backgroundColor: colors.cream.DEFAULT }}>
      ...
      <View style={{
        ...
        {([['today', 'Dziś'], ['medications', 'Leki'], ['stats', 'Statystyki']] as [Tab, string][]).map(
          ([tab, label]) => (
            <Pressable
              key={tab}
        ...
        >
          ...
          <View style={{ flex: 1 }}>
            <Text style={{ fontSize: 13, fontFamily: 'Geist_500Medium', color: colors.ink.DEFAULT }}>
              Zezwól na dokładne alarmy
            </Text>
            <Text style={{ fontSize: 12, color: colors.ink.soft, marginTop: 2 }}>
              Bez tego powiadomienia mogą się opóźniać. Dotknij, by otworzyć ustawienia.
            </Text>
          </View>
      ...
      {activeTab === 'today' && (
        <TodayTab
          ...
          onSnooze={(entry) => {
            ...
            Alert.alert('Odłożono', 'Przypomnę za 15 minut.');
          }}
        />
      )}
      {activeTab === 'medications' && (
        <MedicationsTab
          ...
          onAdd={() => setShowAddModal(true)}
          onDelete={(id) => {
            Alert.alert('Usuń lek', 'Na pewno chcesz usunąć ten lek?', [
    ...
    return (
      <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center', paddingHorizontal: 40 }}>
        ...
        <Text style={{ fontSize: 14, color: colors.ink.faint, textAlign: 'center', marginBottom: 28 }}>
          Dodaj swoje leki i suplementy, żeby śledzić dawkowanie.
        </Text>
        <Pressable
          onPress={onAdd}
          style={{
            ...
            <Pressable
              onPress={() => {
                const entry = todaySchedule.find(
                  (e) =>
                    e.medication.id === nextDue.medication.id &&
                    e.item.id === nextDue.item.id,
                );
  ...
  if (medications.length === 0) {
    return (
      <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center', paddingHorizontal: 40 }}>
        ...
        <Text style={{ fontSize: 17, fontFamily: 'Geist_500Medium', color: colors.ink.DEFAULT, textAlign: 'center', marginBottom: 8 }}>
          Brak leków na liście
        </Text>
        <Text style={{ fontSize: 14, color: colors.ink.faint, textAlign: 'center', marginBottom: 28 }}>
          Dodaj leki, suplementy i witaminy, by otrzymywać przypomnienia.
        </Text>
        <Pressable
          onPress={onAdd}
          style={{
        ...
        >
          <Icon name="plus" size={18} color="#FFFFFF" />
          <Text style={{ fontSize: 15, fontFamily: 'Geist_500Medium', color: '#FFFFFF' }}>Dodaj lek</Text>
        </Pressable>
      </View>
    );
  }
    ...
    return (
      <View style={{ flex: 1, alignItems: 'center', justifyContent: 'center', paddingHorizontal: 40 }}>
        ...
        <Text style={{ fontSize: 14, color: colors.ink.faint, textAlign: 'center', marginTop: 8 }}>
          Zacznij śledzić leki, żeby zobaczyć statystyki.
        </Text>
      </View>
    );
  }
  ...
  return (
    <ScrollView contentContainerStyle={{ paddingHorizontal: 18, paddingBottom: 120 }} showsVerticalScrollIndicator={false}>
        ...
        <View style={{
          ...
          <View>
            ...
            <Text style={{ fontSize: 13, color: colors.ink.soft, marginTop: 2 }}>
              Tak trzymać! Regularność robi różnicę.
            </Text>
          </View>
        </View>
      )}
      ...
      <View style={{ flexDirection: 'row', gap: 10, marginBottom: 24 }}>
        <StatCard label="Dziś" value={todayPct} />
        <StatCard label="Tydzień" value={weekPct} />
        <StatCard label="Miesiąc" value={monthPct} />
      </View>
        ...
        <ScrollView contentContainerStyle={{ paddingHorizontal: 18, paddingTop: 24, paddingBottom: 60 }}>
          ...
          <View style={{
            ...
            <Text style={{ flex: 1, fontSize: 13, color: colors.ink.soft, lineHeight: 19 }}>
              Przypomnienia działają nawet gdy telefon jest zablokowany lub jesteś poza aplikacją.
            </Text>
          </View>

          {/* Toggles */}
          <Text style={[labelStyle, { marginBottom: 12 }]}>Powiadomienia</Text>
          ...
          <View style={{
            ...
            <View style={{
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={{ fontSize: 12, color: colors.ink.faint, marginTop: 2 }}>
                  Przyjemny sygnał przy każdym przypomnieniu
                </Text>
              </View>
              <Switch
                value={prefs.sound}
            ...
            <View style={{
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={{ fontSize: 12, color: colors.ink.faint, marginTop: 2 }}>
                  Wibruje nawet gdy telefon jest wyciszony
                </Text>
              </View>
              <Switch
          ...
          <View style={{
            ...
            <Text style={{ fontSize: 14, color: colors.ink.DEFAULT }}>
              {prefs.sound && prefs.vibration
                ? '🔔 Dźwięk i wibracja włączone'
                : prefs.sound
                ? '🔔 Tylko dźwięk (bez wibracji)'
                : prefs.vibration
                ? '📳 Tylko wibracja (bez dźwięku)'
                : '🤫 Tylko powiadomienie na ekranie'}
            </Text>
          </View>

          {Platform.OS === 'android' && (
            <Text style={{ fontSize: 12, color: colors.ink.faint, textAlign: 'center', marginTop: 20, lineHeight: 18 }}>
              Zmiana ustawień przebuduje harmonogram powiadomień.{'\n'}
...
function AddMedicationModal({
  ...
  const [entries, setEntries] = useState<ScheduleFormEntry[]>([{ time: '08:00', dosage: '1 tabletka' }]);
  const dosageRefs = useRef<Record<number, TextInput | null>>({});

  function reset() {
    setName('');
    setType('supplement');
    setEntries([{ time: '08:00', dosage: '1 tabletka' }]);
  }
  ...
  function handleTimeChange(i: number, raw: string, prevTime: string) {
    ...
    if (rawDigits.length <= 2) {
      formatted = rawDigits;
      // Po wpisaniu 2. cyfry godziny (0-23) automatycznie dodaj ":"
      ...
      // 3-4 cyfry: formatuj jako HH:MM
      formatted = `${rawDigits.slice(0, 2)}:${rawDigits.slice(2, 4)}`;
      // Po kompletnej godzinie (4 cyfry) przenieś focus na dawkę
  ...
  function handleSave() {
    if (!name.trim()) {
      Alert.alert('Uzupełnij nazwę', 'Podaj nazwę leku.');
      return;
    }
    if (entries.length === 0) {
      Alert.alert('Dodaj porę', 'Dodaj przynajmniej jedną porę dawkowania.');
      return;
    }
    ...
    for (let i = 0; i < entries.length; i++) {
      ...
      if (!parsed) {
        Alert.alert('Nieprawidłowa godzina', `Pora nr ${i + 1}: wpisz godzinę w formacie GG:MM (np. 08:30).`);
        return;
      }
      if (!entry.dosage.trim()) {
        Alert.alert('Uzupełnij dawkę', `Podaj dawkę dla pory nr ${i + 1}.`);
        return;
      }
      schedule.push({
        id: `${Date.now()}-${i}`,
        ...
        <SafeAreaView edges={['top']} style={{ flex: 1 }}>
          ...
          <View style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 18, paddingVertical: 14 }}>
            <Pressable onPress={handleClose}>
              <Text style={{ fontSize: 15, color: colors.ink.faint }}>Anuluj</Text>
            </Pressable>
          ...
          <ScrollView contentContainerStyle={{ paddingHorizontal: 18, paddingBottom: 60 }} showsVerticalScrollIndicator={false}>
            {/* Name */}
            <Text style={labelStyle}>Nazwa leku</Text>
            <TextInput
              value={name}
              onChangeText={setName}
              placeholder="np. Kwas foliowy, Magnez, Insulina"
            ...
            {/* Type */}
            <Text style={[labelStyle, { marginTop: 20 }]}>Typ</Text>
            <View style={{ gap: 8 }}>
              {MED_TYPES.map((t) => {
                const cfg = MED_TYPE_CONFIG[t];
                return (
                  <Pressable
                    key={t}
                    onPress={() => setType(t)}
                    style={{
                      flexDirection: 'row', alignItems: 'center', gap: 12,
                      backgroundColor: type === t ? colors.evergreen.DEFAULT : colors.surface.DEFAULT,
                      borderRadius: 12, borderWidth: 0.5,
                      borderColor: type === t ? colors.evergreen.DEFAULT : colors.line.DEFAULT,
                      padding: 12,
                    }}
                  >
                    <View style={{
                      width: 36, height: 36, borderRadius: 10,
                      backgroundColor: type === t ? 'rgba(255,255,255,0.18)' : cfg.bgColor,
                      alignItems: 'center', justifyContent: 'center',
                    }}>
                    ...
                    <Text style={{
                      ...
                      color: type === t ? '#FFFFFF' : colors.ink.DEFAULT,
                    }}>
                      {cfg.label}
                    </Text>
                    {type === t && (
            ...
            {/* Schedule */}
            <Text style={[labelStyle, { marginTop: 24 }]}>Harmonogram dawek</Text>
            <View style={{ gap: 8 }}>
              {entries.map((entry, i) => (
                <View
                  key={i}
