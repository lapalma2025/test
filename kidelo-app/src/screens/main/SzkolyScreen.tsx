      ...
      <View className="px-5 pt-3 pb-2">
        ...
        {!isLoading && (
          <Text className="text-ink-faint text-[12px] mt-1">
            {resultCount} {tab === 'schools' ? 'szkół' : 'szpitali'}
            {voivodeshipFilter === 'all' ? ' · cała Polska' : ` · ${voivodeshipFilter}`}
          </Text>
        )}
      </View>
      ...
      <View className="px-5 mb-2">
        <View className="flex-row items-center gap-2 bg-surface border border-line rounded-card px-3 py-2">
          ...
          <TextInput
            ...
            placeholder={tab === 'schools' ? 'Szukaj szkoły…' : 'Szukaj szpitala…'}
            placeholderTextColor={colors.ink.faint}
            className="text-ink text-[13px] flex-1 py-0"
          />
          {search.length > 0 && (
      ...
      {viewMode === 'map' && (
        isLoading ? (
          <View className="flex-1 items-center justify-center gap-2">
            ...
            <Text className="text-ink-faint text-[12px]">Pobieram placówki…</Text>
          </View>
        ) : items.length === 0 ? (
          <EmptyState tab={tab} />
        ) : (
      ...
      <Pressable onPress={onOpen} className="p-4 active:opacity-80">
        <View className="flex-row justify-between items-start mb-2">
          <View className="flex-1 mr-3">
            ...
            <View className="flex-row items-center gap-1.5 mt-1.5 flex-wrap">
              <Pill tone={school.is_nfz_free ? 'sage' : 'mustard'}>
                {school.is_nfz_free ? 'NFZ' : 'Prywatna'}
              </Pill>
              <Pill tone="neutral">
                {school.type === 'online' ? 'Online' : school.type === 'hybrid' ? 'Hybryda' : 'Stacjonarna'}
              </Pill>
              {school.city && (
          ...
          <Text className={`font-mono text-[13px] ${school.is_nfz_free ? 'text-sage' : 'text-ink'}`}>
            {school.is_nfz_free
              ? 'Bezpłatna'
              : school.payment ?? (school.price_pln ? `${school.price_pln} zł` : 'Płatna')}
          </Text>
        </View>
      ...
      <View className={`flex-row border-t ${selected ? 'border-sage' : 'border-line'}`}>
        ...
        {hasMap && (
          ...
          >
            ...
            <Text className="text-evergreen text-[11px] font-sans-medium">Mapa</Text>
          </Pressable>
        )}
      </View>
    </View>
      ...
      <Pressable onPress={onOpen} className="p-4 active:opacity-80">
        ...
        <View className="flex-row items-center gap-1.5 mt-1.5 flex-wrap">
          <Pill tone={hospital.is_nfz !== false ? 'sage' : 'mustard'}>
            {hospital.is_nfz !== false ? 'NFZ' : 'Prywatny'}
          </Pill>
          {hospital.city && (
            <View className="flex-row items-center gap-1">
              <Icon name="pin" size={11} color={colors.ink.faint} />
        ...
        <View className="flex-row items-center gap-3 mt-2">
          {hospital.has_anesthesia && <Pill tone="sage">Znieczulenie</Pill>}
          {hospital.rating && (
            <View className="flex-row items-center gap-1">
              <Icon name="star" size={12} color={colors.terracotta.DEFAULT} />
...
function EmptyState({ tab }: { tab: TabKey }) {
  return (
    <View className="flex-1 items-center justify-center gap-3 px-8">
      ...
      <Text className="text-ink font-sans-medium text-[15px] text-center">Brak wyników</Text>
      <Text className="text-ink-soft text-[12px] text-center leading-snug">
        {tab === 'schools'
          ? 'Spróbuj „Cała Polska”, usuń filtr NFZ lub wpisz inne miasto.'
          : 'Wybierz województwo lub „Cała Polska”. Dane z lokalnej bazy placówek.'}
      </Text>
    </View>
  );
}
