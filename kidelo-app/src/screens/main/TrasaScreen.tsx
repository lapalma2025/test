/**
 * TrasaScreen.tsx — główny ekran aplikacji.
 *
 * Pokazuje bieżący etap (lub wybrany przez użytkownika) z timeline-generator.
 * Pasek 8 etapów jest klikalny — pozwala przeglądać wcześniejsze i późniejsze tygodnie.
 */
...
// Oblicz tydzień ciąży z terminu porodu (termin = 40 tygodni)
...
// Tydzień dla danego etapu (stageId): aktualny jeśli bieżący, ostatni tygodniu jeśli przeszły, pierwszy jeśli przyszły
...
export default function TrasaScreen() {
  ...
  const { timeline, results, totalProjected } = useMemo(() => {
    ...
    const timeline = generateTimeline(userProfile, results, now);
    ...
    profile.partnerEmployment, profile.firstChild, profile.hasDisability,
  ]);

  // Tydzień ciąży
  const currentPregnancyWeek = calcPregnancyWeek(profile.childDueDate, now);

  // Indeks aktualnie PRZEGLĄDANEGO etapu (może być inny niż bieżący)
  ...
  if (!viewStage) {
    return (
      <MainScreenShell className="items-center justify-center">
        <Text className="text-ink-soft">Brak danych. Wypełnij onboarding.</Text>
      </MainScreenShell>
    );
  }
    ...
    <MainScreenShell>
      ...
      >
        ...
        <View className="px-5 pt-2 pb-3 flex-row justify-between items-start">
          <View>
            <Text className="text-ink text-[17px] font-sans-medium">
              Dzień dobry{profile.parentName ? `, ${profile.parentName}` : ''}
            </Text>
        ...
        <View className="px-5 py-3">
          ...
          {/* Pasek etapów — każdy jest klikalny */}
          ...
          {/* Nawigacja prev/next + licznik + powrót do teraz */}
            ...
            <View className="flex-1 items-center px-2">
              {isBrowsing ? (
                <Pressable onPress={goToCurrent} className="flex-row items-center gap-1.5 active:opacity-70">
                  ...
                  <Text className="text-evergreen text-[12px] font-sans-medium">Wróć do teraz</Text>
                </Pressable>
              ) : (
                <View className="flex-row items-center gap-1.5">
                  <View className="w-1.5 h-1.5 rounded-full bg-evergreen" />
                  <Text className="text-evergreen text-[12px] font-sans-medium">Twój etap</Text>
                </View>
              )}
              <Text className="text-ink-faint text-[11px] mt-0.5">
                etap {viewIdx + 1} z {timeline.stages.length} · {viewStage.band}
              </Text>
            </View>
          ...
          {/* Badge "przeglądasz przeszłość/przyszłość" */}
          {isBrowsing && (
            <View className={`flex-row items-center gap-1.5 mt-3 self-start px-3 py-1.5 rounded-full ${
              ...
              <Text className={`text-[11px] font-sans-medium ${
                ...
                {viewIdx < currentIdx ? 'przeglądasz przeszłość' : 'przeglądasz przyszłość'}
              </Text>
            </View>
          )}
        </View>
          ...
          >
            <View className="flex-row justify-between items-center mb-2.5">
              <Text className="text-cream/70 text-[11px] uppercase tracking-wider font-sans-medium">
                {isBrowsing
                  ? viewIdx < currentIdx ? 'już za Tobą' : 'nadchodzi'
                  : viewStage.hero.urgent ? 'pilne teraz' : 'teraz'}
              </Text>
              {!isBrowsing && viewStage.hero.urgent && (
                <View className="bg-mustard px-2 py-0.5 rounded-full flex-row items-center gap-1">
                  ...
                  <Text className="text-[10px] font-sans-medium" style={{ color: '#2C1A0A' }}>termin blisko</Text>
                </View>
              )}
            </View>
            ...
            {viewStage.hero.link && (
              <View className="flex-row gap-2">
                <View className="bg-cream/15 px-4 py-2 rounded-card flex-row items-center gap-1.5">
                  <Text className="text-cream font-sans-medium text-[13px]">Otwórz</Text>
        ...
        <View className="px-5 pt-6">
          <View className="flex-row justify-between items-baseline mb-3">
            <Text className="text-ink text-[15px] font-sans-medium">
              {isBrowsing ? 'Zadania w tym etapie' : 'Następne 30 dni'}
            </Text>
            <Pressable onPress={() => router.push('/timeline' as any)} className="active:opacity-60">
              <Text className="text-sage text-[12px] font-sans-medium">Pełna oś czasu →</Text>
            </Pressable>
          </View>
        ...
        {/* Karta tygodnia ciąży — widoczna tylko dla etapów t1/t2/t3 */}
        ...
        <View className="px-5 pt-6">
          ...
          >
            ...
            <Text className="text-ink-soft text-[12px] mt-1">
              {results.filter((r) => ['eligible', 'action', 'active'].includes(r.eligibility)).length} świadczeń · projekcja
            </Text>
            <View className="flex-row items-center gap-1 mt-3">
              <Text className="text-sage text-[13px] font-sans-medium">Zobacz świadczenia</Text>
...
function WeekCard({
  ...
  return (
    <View className="px-5 pt-5">
      <Text className="text-ink text-[15px] font-sans-medium mb-3">Twój tydzień ciąży</Text>
      <Pressable
      ...
      >
          ...
          <View className="flex-1">
            {isCurrentStage ? null : (
              <Text className="text-ink-faint text-[10px] uppercase tracking-wide mb-1">
                {isPastStage ? 'ostatni tydzień etapu' : 'pierwszy tydzień etapu'}
              </Text>
            )}
        ...
        {(weekObj.rozmiar_dziecka.dlugosc !== '—' || weekObj.rozmiar_dziecka.waga !== '—') && (
          <View className="flex-row px-4 pb-3 gap-5">
            {weekObj.rozmiar_dziecka.dlugosc !== '—' && (
              <View>
                <Text className="text-ink-faint text-[10px] uppercase">Długość</Text>
            ...
            {weekObj.rozmiar_dziecka.waga !== '—' && (
              <View>
                <Text className="text-ink-faint text-[10px] uppercase">Masa</Text>
                <Text className={`font-sans-medium text-[13px] ${trimText}`}>
                  {weekObj.rozmiar_dziecka.waga}
                </Text>
              </View>
        ...
        <View className="bg-white/50 px-4 py-3 flex-row items-center gap-3">
          ...
          <View className="flex-row items-center gap-1 shrink-0">
            <Text className="text-evergreen text-[12px] font-sans-medium">Więcej</Text>
